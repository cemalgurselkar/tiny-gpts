...
class GPT(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.decoder = nn.ModuleDict(dict(
            ...
            layer_f = LayerNorm(config.n_embd, bias=config.bias)
        ))
        ...
        for pn, p in self.named_parameters():
            if pn.endswith('c_project.weight'):
                torch.nn.init.normal_(p, mean=0.0, std=0.02/math.sqrt(2 * config.n_layer))

        print(f"number of parameters: %{self.get_num_params()/1e6:.2f}M")
    
    def get_num_params(self, non_embedding=None):
        n_params = sum(p.numel() for p in self.parameters())
        if non_embedding:
            n_params -= self.transformer.wpe.weight.numel() #numel is PyTorch tensor method that return the total number of element. 
        return n_params
    ...
    def forward(self, idx, targets=None):
        ...
        tok_embedding = self.decoder.wte(idx) # token embedding
        pos_embedding = self.decoder.wpe(pos) # positional embedding
        x = self.decoder.drop(tok_embedding + pos_embedding)
        for block in self.decoder.h:
            x = block(x)
        x = self.decoder.layer_f(x)
